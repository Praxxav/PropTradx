...
const Pricing = () => {
  // ✅ Set initial state directly (prevents SSR mismatch)
  const [accountType, setAccountType] = useState<string>("Standard")
  const [evaluationPhase, setEvaluationPhase] = useState<string>("3-Step")
  ...
  return (
    <section id="Pricing" className="w-full py-16 px-6 flex flex-col md:flex-row items-center justify-between max-w-6xl mx-auto">
      <motion.div
        ...
        <motion.div className="flex gap-4 mb-6 flex-wrap">
          {accountBalances.map((balance) => (
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
            >
              ${balance.toLocaleString("en-US")} {/* ✅ Ensure consistent formatting */}
            </motion.button>
          ))}
        </motion.div>
